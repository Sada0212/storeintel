 ...
 * Version: renderer_opp_pwa_v1.1.js
 * Date:    2026-07-28
 *
...
function oppKpi(val, label, sub) {
  return `<div class="kpi" style="border-left:3px solid var(--teal,#1A7A7A)">
    <div class="kpi-val" style="color:var(--teal,#1A7A7A)">${val}</div>
...
function oppKpiRed(val, label, sub) {
  return `<div class="kpi" style="border-left:3px solid var(--red,#8b2020)">
    <div class="kpi-val" style="color:var(--red,#8b2020)">${val}</div>
    <div class="kpi-lbl">${label}</div>
    ${sub ? `<div style="font-size:11px;color:var(--muted,#6b7280);margin-top:2px">${sub}</div>` : ''}
  </div>`;
}
...
function oppBuildOverview(ov) {
  const reasonCols = {
    DESIGN:'var(--teal,#1A7A7A)', PRICE:'var(--red,#8b2020)',
    SIZE:'var(--green,#1a6b45)', WEIGHT:'var(--orange,#b85c1a)', OTHER:'#888'
  };
    ...
    <div class="page">
      <div class="section">
        ...
        <div class="kpi-grid" style="grid-template-columns:repeat(2,1fr)">
          ...
          ${oppKpi(ov.avg_value_fmt, 'Avg per Footfall', '')}
          ${oppKpi(`${ov.recoverable_count} (${ov.recoverable_pct}%)`, 'Recoverable', 'Product was shown')}
...
function oppBuildOpportunity(opp) {
  ...
  const cardRows = allCards.map(c => {
    ...
    return `<tr class="opp-card-row" style="cursor:pointer" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'table-row':'none'">
    ...
    <div class="page">
      <div class="section">
        ...
        <div class="kpi-grid" style="grid-template-columns:repeat(2,1fr)">
          ...
          ${oppKpi(`${opp.p2_count} customers`,'P2 — Follow Up', opp.p2_value_fmt)}
          ${oppKpi(`${opp.p3_count} customers`,'P3 — When Stocked', opp.p3_value_fmt)}
          ${oppKpi(`${opp.competitor_count}`,'Competitor Flagged','Needs different message')}
        </div>
...
function renderOppReportPWA(results, storeName) {
  ...
  console.log('[OPP PWA v1.1] Rendered:', ov.total_footfalls, 'records,',
    oppTabs.length, 'tabs added, click listeners attached');
}
